...
import br.com.marcacao.peladas.context.WebApplicationInitializer;

@Configuration
@EnableJpaRepositories(value = "br.com.marcacao.peladas.repository")
public class JPAConfiguration {
	...
	/**
	 * {@link JpaTransactionManager} also supports direct DataSource access within a
	 * transaction (i.e. plain JDBC code working with the same DataSource). This
	 * allows for mixing services which access JPA and services which use plain JDBC
	 * (without being aware of JPA)! Application code needs to stick to the same
	 * simple Connection lookup pattern as with DataSourceTransactionManager (i.e.
	 * DataSourceUtils.getConnection(javax.sql.DataSource) or going through a
	 * TransactionAwareDataSourceProxy). Note that this requires a vendor-specific
